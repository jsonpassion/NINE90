...
앱의 화면 흐름과 기능은 [USER_FLOW.md](USER_FLOW.md) 참고.
...
## 챕터 안정성 규칙 (2026-09-10, 앱 빌드 997+)

앱은 단어 ID를 `<unit id>-<headword slug>`로 만든다 (예: `voca-600-799-u003-compliance`).
줄 번호가 아니라 표제어에서 나오므로, 파일 안에서 줄을 끼워 넣거나 지우거나 순서를 바꿔도
다른 단어의 학습 진도가 밀리지 않는다. 무료 구간은 정본 순서의 앞 100단어(= unit-001)로 고정되고,
회독 중 순서는 기기에 고정(pin)되어 있어 새 단어는 항상 마지막 챕터 뒤에 붙는다.

이 보장이 유지되려면 편집할 때 지킬 것:

- **새 권은 새 파일로**: `unit-011.md`, `unit-012.md`처럼 번호를 이어서 추가한다. 100단어 단위가 아니어도 되지만
  100단어 = 10챕터 = 1권이 책장에 맞는다.
- **표제어(첫 필드)는 바꾸지 않는다**: 표제어를 고치면 ID가 바뀌어 그 단어의 진도가 새 단어로 취급된다.
  뜻, 발음, 힌트, 예문, 번역은 자유롭게 고쳐도 된다.
- **같은 파일에 같은 표제어 두 번은 피한다**: 앱이 `-2` 접미사로 구분은 하지만, 두 번째 항목을 지우면 그 진도만 사라진다.
- **unit-001은 무료 권이다**: 이 파일의 단어를 바꾸면 무료 단어가 바뀐다. 의도한 경우에만 편집한다.
- 편집 후 `python3 tools/build_manifest.py`로 매니페스트를 다시 만들고 커밋한다. 앱은 버전 해시 변경을 감지해 받아 간다.

앱 쪽 증명은 DEBUG 실행 인자 `-poolOrderCheck`로 돌릴 수 있다 (14개 불변식, 시뮬레이터 로그 `category == "pool"`).
